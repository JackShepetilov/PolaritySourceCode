...
Игрок **постоянно** зафиксирован на (+). Ручное переключение полярности убрано из дизайна (упрощение по фидбеку — слишком много кнопок и полярность в голове). Заряд игрока — **ресурс**: копится в ближнем бою, тратится на способности. «Cause and Effect»-арена (полярность окружения) заморожена вместе с отказом от переключения. Легаси-гейт: `EMFVelocityModifier::bAllowPolarityToggle` (по умолчанию `false`).
...
> **Упрощённая модель (текущая версия):** игрок постоянно «+», объекты (NPC, пропы,
> оружие) электризуются «−» — поэтому их всегда можно захватить. **Переключения
> полярности нет** — убрано как усложнение (много кнопок / полярность в голове).
> Заряд игрока — ресурс: копится в ближнем бою, тратится на способности. Механика
> «Cause and Effect» (полярность окружения) заморожена. Легаси-гейт:
> `EMFVelocityModifier::bAllowPolarityToggle` (по умолчанию `false`).
...
### 2.2 Переключение полярности — ВЫРЕЗАНО (легаси)

**Убрано из дизайна** (упрощение: слишком много кнопок / полярность в голове). Игрок
постоянно «+». Идея «полярность окружения» («Cause and Effect») заморожена — может
вернуться отдельной механикой. Ниже — легаси-параметры на случай возврата.
...
### 4.4 Порядок удара и кулдаун

Зарядной системы больше нет (убрана 2026-09-10). Удар устроен так:

1. Нажатие: ствол и руки исчезают **в этот же кадр**, без анимации холстера и без опускания.
2. На `MeleeMesh` играет монтаж удара **целиком**. Окно урона задаёт нотифай
   `AnimNotifyState_MeleeDamageWindow` в самом монтаже, своих таймеров у удара нет.
3. Монтаж начал затухать: `MeleeMesh` пропадает, ствол достаётся своей анимацией Draw.
   Скорость доставания: `DrawSpeedMultiplier` на компоненте (множитель к анимации оружия, для апгрейдов).
4. Новый удар во время Draw НЕЛЬЗЯ: компонент сидит в `ShowingWeapon`, пока Draw не кончится.
   Смена оружия Draw прервать может, как любой другой Draw.

| Параметр | Значение |
|----------|----------|
| Settings.Cooldown | 0.5s, только после удара, попавшего во врага |
