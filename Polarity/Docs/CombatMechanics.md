...
### 7.1 Броня игрока (DOOM Eternal-style)

Броня — дополнительный слой защиты, поглощающий 100% входящего урона до полного истощения. Только после этого урон идёт в HP.

| Параметр | Значение |
|----------|----------|
| MaxArmor | 250 |
| StartArmor | 0 (зарабатывается через channeling) |
| Поглощение | 100% урона пока есть броня |
...
Дропаются с NPC при убийстве через **channeling** — захват и/или бросок. Конкретно:
- Расстрел захваченного NPC (bIsCaptured)
- Бросок NPC в стену (wallslam kill после launch)
...
Пикапы (HP и броня) используют прямое преследование — каждый кадр двигаются строго к игроку без инерции. Скорость нарастает квадратично от 10% до 100% MagnetSpeed. Невозможно промахнуться мимо игрока.
...
### Петля 4: Канализация → Захват → Уничтожение захваченного → Броня
...
> Убийство дропает **броню** (не хилку) — channeling kill

### Петля 5: Канализация → Захват NPC → Запуск → Броня
> Удерживай → пластина → захват NPC
> Отпусти + тап → реверс → NPC летит как снаряд
> NPC → стена = wallslam урон → дроп **брони**
> NPC → другой NPC = collision урон → **оба** дропают **броню**
