...
// ==================== Intro Cutscene (boss-level run start) ====================

void AShooterCharacter::BeginIntroCutscene()
{
	// Hide the player entirely — the intro's cinematic camera frames the boss, the player must not
	// be seen. Unlike the finisher (which hides only the FP body + uses a double), also hide the
	// third-person mesh so a cine camera can't catch the body. No teleport: player is pre-placed.
	if (CurrentWeapon)
	{
		CurrentWeapon->StopFiring();
	...
	// The cine camera (Camera Cuts) owns the view; kill procedural camera animation so it can't fight it.
	if (APlayerController* PC = Cast<APlayerController>(GetController()))
	{
		if (PC->PlayerCameraManager)
		{
	...
	UE_LOG(LogTemp, Log, TEXT("[RUN_DEBUG] BeginIntroCutscene — player hidden + input locked"));
}

void AShooterCharacter::EndIntroCutscene()
{
	// Reveal + restore. The camera blend back to the player is done in BP (SetViewTargetWithBlend);
	// here we just un-hide, restore input, and draw the starting weapon with the smooth raise.
	if (UCameraShakeComponent* Shake = GetCameraShake())
	{
	...
	if (USkeletalMeshComponent* TPMesh = GetMesh())
	{
		TPMesh->SetVisibility(true, /*bPropagateToChildren=*/ true);
	}
	if (USkeletalMeshComponent* FPMesh = GetFirstPersonMesh())
	{
	...
	// Draw the starting weapon (animated raise). The equip re-asserts FP-mesh/weapon visibility.
	EquipStartingWeaponAnimated();

	UE_LOG(LogTemp, Log, TEXT("[RUN_DEBUG] EndIntroCutscene — player revealed + weapon raised"));
}
