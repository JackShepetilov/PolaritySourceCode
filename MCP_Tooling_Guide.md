...
## 1.0b ОДИН РЕДАКТОР — ОДИН АГЕНТ (крэш 2026-06-11)

Редактор убит наповал командой `build_arena` (load_level), прилетевшей, пока редактор держал
ДРУГОЙ агент (LandscapeService-пробы, открытый TestRun): лог обрывается на команде без
Fatal-строки. Правила: (1) перед любой сборкой/сменой уровня — сверка с автором, кто сейчас
владеет редактором; (2) `build_arena.py` теперь сам отказывается работать при активном PIE
(`ensure_safe_to_build`); (3) гонку двух агентов код не ловит — только очерёдность через автора.

**Смена карты скриптом в ДОЛГОЙ сессии = крэш** (2026-06-11, второй труп редактора):
`load_level` на тяжёлую карту после часов python-работы упал в Fatal `EditorServer.cpp:1934`
(world-leak check — накопленные ссылки на акторы старого мира). Загрузка уровня скриптом
безопасна только в СВЕЖЕЙ сессии (пайплайны так и делают). Кросс-карточное чтение (сравнить
конфиги двух карт) — ТОЛЬКО отдельным headless-инстансом при закрытом редакторе (образец:
`Build/demo_water_dump.log`-прогон). Эталон воды: зона ВЫШЕ пиков рельефа (демо: 4120 над
своими горками; остров: 17500 над цитаделью 16000), extent ~3× карты, сплайн океана НЕ
ограничивает поверхность — оставлять маленьким.

**Скриптовый `load_level` МОЛЧА сбрасывает несохранённые правки** (без диалога «Save?»), а
финальный `save_dirty_packages` цементирует потерю — так 2026-06-11 был затёрт свежедобавленный
авторский сублевел ArenaLightingDebug3 + BP_ShooterCharacter на Lvl_IslandSpike (автор вернул
руками). Фикс: `open_or_create_level` в `build_arena.py` теперь сам зовёт
`EditorLoadingAndSavingUtils.get_dirty_map_packages()` и **отказывается работать при непустом
списке** («сохранись в редакторе, потом запускай»). Любой НОВЫЙ скрипт с загрузкой уровней
обязан идти через этот хелпер или повторять гард. Плюс у спайка `ensure_author_extras`:
известные авторские сублевелы прицепляются обратно, если их вдруг нет.

⚠️ **Стартовая карта TestLevel авто-дертится через ~1 мин после запуска редактора**
(construction-script сайд-эффекты; поймано 2026-06-12: пробник видел пустой dirty-список, через
21 с гард сборки упал на TestLevel, которого никто не трогал). Лечение без участия автора:
`py Tools/ArenaBlockout/save_testlevel_if_only_dirty.py` — сохраняет ТОЛЬКО если dirty ровно
один TestLevel (сохранение ничего не теряет), на любой другой список громко отказывается.
...
- **Спавн акторов идёт в ТЕКУЩИЙ уровень редактора, а `add_level_to_world*` делает
  свежеприцепленный сублевел текущим** (поймано 2026-06-11: мосты острова уехали в пакеты
  Lvl_A6_Villa и Lvl_A1_Pier). В сборных скриптах: ВСЕ спавны — ДО прицепления сублевелов,
  либо явный `EditorLevelUtils.make_level_current(streaming_level)` обратно на персистент.
...
- **LandscapeService: скилл-док врёт в kwargs** (поймано 2026-06-11): у `create_mountain`/
  `create_valley`/`create_ridge` параметр шума зовётся `add_noise`, НЕ `b_add_noise` из skill.md.
  Перед первым вызовом сервиса — probe `fn.__doc__` (точные сигнатуры v3 в одну строку).
  `landscape_exists()` проверяет ТЕКУЩИЙ открытый мир, не диск. Скульпт-операции АДДИТИВНЫ —
  повторный прогон удваивает рельеф; идемпотентность = flatten-reset до базовой плоскости.
- **В редакторе может работать ВТОРАЯ Claude-сессия** (поймано 2026-06-11: чужой батч пересборки
  арен перекрыл уровень между моими командами — несохранённый ландшафт пропал). Перед серией
  мутаций уровней: `get_output_log` на чужие свежие `[ARENA_BLOCKOUT]`/`[LEVEL_SNAP]`/`Cmd:`
  и согласование с автором, чья очередь. Несохранённое = потеряно при первом чужом load_level.
- `EditorLevelUtils.make_level_current` принимает ТОЛЬКО LevelStreaming (ULevel падает конверсией
  параметра); персистент делается текущим через
  `LevelEditorSubsystem.set_current_level_by_name("<короткое имя карты>")` (проверено 2026-06-12).
- **FoliageService кладёт инстансы в ПЕРВЫЙ найденный в мире InstancedFoliageActor**, а НЕ в IFA
  текущего уровня (поймано 2026-06-12 дважды: 3775 инстансов уехали в авторский Lvl_RunLogic;
  повторная посадка при живом чужом IFA легла туда же, а «зачистка мусора» после посадки снесла
  свежие инстансы вместе с актором). Рецепт: чужие/пустые IFA сносить ДО add_foliage_instances
  (при пустом мире сервис создаёт IFA в ТЕКУЩЕМ уровне), после посадки верифицировать пакет IFA
  + суммарный счётчик `list_foliage_types`, и только потом сохранять (образец:
  `Tools/ArenaBlockout/apply_biome1_art_pass.py`, phase 3).
- **Synty-ландшафт чёрный = слой `Base` без НЕНУЛЕВОГО веса** (поймано 2026-06-12 на
  `M_Landscape_base_Inst`, дважды): все пейнт-слои мастера — LB_HeightBlend, `Base` —
  LB_AlphaBlend; без вклада Base ВЕСЬ ландшафт рендерится чисто-чёрным при корректных весах
  остальных слоёв (данные читаются, compile молчит). Голая аллокация НЕ спасает: при попиксельной
  сумме остальных масок ровно 255 Base уходит в 0, UE выбрасывает пустую аллокацию при
  сохранении, рендер снова чёрный, а следующий `import_weight_map("Base")` падает
  «Layer not found». Рабочий рецепт: маски слоёв печь на 229, Base держать ~26 (10%) везде —
  и Base импортировать ПЕРВЫМ (⚠️ `import_weight_map` сваливает дефицит нормализации в ПЕРВЫЙ
  слой ландшафта по порядку назначения: импорт Base ПОСЛЕ остальных перетёр Grass→Sand по
  всей карте). Итоговый пиксель 0.898/0.102 — примесь визуально невидима. Образец:
  `apply_biome1_art_pass.py` (phase 2) + `make_biome1_weightmaps.py` (BINV=229).
...
- `ULandscapeMaterialService.cpp::AssignMaterialToLandscape` (2026-06-12): **фикс потери слоёв
  ландшафта при сохранении** — сервис регистрировал слои только в ТРАНЗИЕНТНОМ `ULandscapeInfo`
  (пересобирается пустым при каждой загрузке карты), сериализуемый реестр
  `ALandscapeProxy::TargetLayers` оставался пустым → каждый сейв терял ВСЕ привязки слоёв
  (на загрузке/PIE-дупликации/standalone материал падал в preview-фоллбек: «всё травой
  Base»/чёрный, при идеальной картинке в живой сессии). Патч зеркалирует привязки в
  `TargetLayers` через официальный API 5.5+ (`Add/UpdateTargetLayer`). Диагностика бага:
  `ls.get_editor_property("target_layers")` == `{}` при живых слоях; лечится только
  пересборкой плагина (свойство read-only из python, мутаторов в reflection нет).
...
4. **Скриншоты врут.** Вьюпорт с выключенным realtime НЕ перерисовывается — повторные
   ...
   вьюпорт-скриншот НЕ попадает. **Ландшафт сразу после `import_heightmap` рендерится грубыми
   террасами-кубами** (рендер-данные 2017² перестраиваются асинхронно несколько секунд) — снимок
   через 1–2 с показывает «8-битное» месиво при идеальных данных; ждать ≥8 с, а данные проверять
   `get_height_at_location`, не глазами (ложная паника 2026-06-11).
