...
- **Synty-ландшафт чёрный = неаллоцированный `Base`-слой** (поймано 2026-06-12 на
  `M_Landscape_base_Inst`): все пейнт-слои мастера — LB_HeightBlend, `Base` — LB_AlphaBlend;
  пока Base не аллоцирован на ландшафте, ВЕСЬ ландшафт рендерится чисто-чёрным при корректных
  весах остальных слоёв (данные читаются, compile молчит). Лечение: LayerInfo для Base + импорт
  его маски. ⚠️ **`import_weight_map` сваливает дефицит нормализации в ПЕРВЫЙ слой ландшафта**
  (импорт Base ПОСЛЕ остальных перетёр Grass→Sand по всей карте): Base импортировать ПЕРВЫМ,
  затем настоящие маски (попиксельная сумма 255 оставит Base=0, аллокация сохраняется —
  рендер ок; проверено скриншотами). Образец: `apply_biome1_art_pass.py`, phase 2.
